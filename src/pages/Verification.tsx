...
import {
  ...
  CheckCircle, Clock, XCircle, FileImage, User
} from 'lucide-react';
...
const Verification = () => {
    ...
    const fetchVerification = async () => {
      ...
      if (data) {
        ...
        if (data.id_last_name) {
          setFormData({
            id_last_name: data.id_last_name || '',
            id_first_name: data.id_first_name || '',
            id_middle_name: data.id_middle_name || '',
            id_date_of_birth: data.id_date_of_birth || '',
            id_sex: data.id_sex || '',
            id_blood_type: data.id_blood_type || '',
            id_marital_status: data.id_marital_status || '',
            id_place_of_birth: data.id_place_of_birth || '',
          });
        }
      }
      setLoading(false);
  ...
  const isPending = verification?.philsys_status === 'pending' && verification?.id_front_url;
  ...
  const handleDocumentUpload = async () => {
    ...
    await supabase.from('audit_trail').insert({
      ...
      details: 'ID front and back uploaded',
    });

    setUploading(false);
    if (!error) {
      toast.success('Documents uploaded successfully!');
      setVerification(prev => prev ? { ...prev, id_front_url: frontPath, id_back_url: backPath, philsys_status: 'pending', biometric_status: 'pending' } : null);
    }
  };

  // Phase 2: Save manual data
  const handleSaveDetails = async () => {
    ...
    const { error } = await supabase.from('verifications').update({
      ...
      id_middle_name: formData.id_middle_name || null,
      id_date_of_birth: formData.id_date_of_birth,
      id_sex: formData.id_sex,
      id_blood_type: formData.id_blood_type || null,
      id_marital_status: formData.id_marital_status || null,
      id_place_of_birth: formData.id_place_of_birth || null,
    } as any).eq('id', verification.id);

    await supabase.from('audit_trail').insert({
      user_id: user.id,
      action: 'id_details_entered',
      event_type: 'verification',
      details: `Manual data entry: ${formData.id_first_name} ${formData.id_last_name}`,
    });
  ...
  const handleBiometricSubmit = async () => {
    ...
    setVerification(prev => prev ? { ...prev, biometric_status: 'pending', selfie_url: selfiePath, liveness_result: true, face_match_score: faceMatchScore } : null);
  };
  ...
  if (loading) {
    return <div className="container py-8"><div className="animate-pulse space-y-4 max-w-2xl mx-auto"><div className="h-8 bg-muted rounded w-1/2" /><div className="h-64 bg-muted rounded" /></div></div>;
  }
  ...
  return (
    <div className="container py-8 max-w-3xl">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Complete all 3 phases to unlock full marketplace access</p>
      </div>
        ...
        <div className="mb-6 p-4 rounded-lg border border-destructive/30 bg-destructive/5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Please re-submit your documents.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Waiting for review */}
      {isWaitingReview && !isFullyVerified && (
        <Card className="mb-6">
          <CardContent className="p-6 text-center">
            <Clock className="h-12 w-12 text-yellow-500 mx-auto mb-3" />
            <h2 className="text-xl font-bold mb-2">Under Review</h2>
            <p className="text-muted-foreground">Your documents are being processed. Admin approval typically takes 1–3 business days.</p>
          </CardContent>
        </Card>
      )}
      ...
      {!isFullyVerified && !isWaitingReview && (
        <>
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-muted-foreground mt-2">Phase {currentPhase + 1} of {PHASES.length}</p>
          </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        {idFrontFile ? (
                          <span className="text-sm text-muted-foreground">{idFrontFile.name}</span>
                        ) : (
                          <>
                            <Upload className="h-8 w-8 text-muted-foreground mx-auto mb-2" />
                            <p className="text-xs text-muted-foreground">Front of ID</p>
                          </>
                        )}
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        {idBackFile ? (
                          <span className="text-sm text-muted-foreground">{idBackFile.name}</span>
                        ) : (
                          <>
                            <Upload className="h-8 w-8 text-muted-foreground mx-auto mb-2" />
                            <p className="text-xs text-muted-foreground">Back of ID</p>
                          </>
                        )}
            ...
            <CardContent>
              {currentPhase < 2 ? (
                ...
                <div className="space-y-4">
                  ...
                  <Button
                    className="w-full"
                    disabled={!livenessPassed || !selfieBlob || submittingBiometric}
